...
.cclass tr:first-child td:last-child{
	border-width:0px 0px 1px 1px;
		...
		<?php include_once("header/atas_bukti_transfer.php"); ?>	
	...
	<?php
	$no=1;
	$q="SELECT *,DATE_FORMAT(waktu,'%d-%m-%Y') AS waktu2 FROM penjualan p,kota a,provinsi b,nama_jasa_pengiriman c where p.id_kota=a.id_kota and p.id_provinsi=b.id_provinsi and p.id_nama_jasa=c.id_nama_jasa and p.status=1 and p.id_member=$_SESSION[id1]";
	...
	while($row=mysql_fetch_array($result))
	{
		...
		echo "<tr>";
			...
			echo "<td style='text-align:right'>".$row['waktu2']."</td>";
			echo "<td style='text-align:right'> Rp ".number_format($row['jumlah'],0,".",".").",-</td>";
			echo "<td style='text-align:right'> Rp ".number_format($row['harga_pengiriman'])."</td>";
			...
			echo "<td><a href='buktitransfer/simpan_session.php?id_penjualan=".$row['id_penjualan']."'><center><button type='button' class='btn-custom5 btn-small'><i class='icon-camera icon-white'></i></button></center></a>  "."</td>";
		echo "</tr>";
		$no++;
	}
}
